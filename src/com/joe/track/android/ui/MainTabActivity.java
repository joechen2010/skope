...
import com.joe.track.android.application.ServiceQueue;
...
public class MainTabActivity extends TabActivity {
	...
	private TrackApplication mApplication;
	...
	public void onCreate(Bundle savedInstanceState) {
	    ...
	    mApplication = (TrackApplication) getApplication();
	...
	protected boolean checkCacheSanity() {
		...
		if (mCache.isUserSignedOut()) {
			...
			i.setClassName("com.joe.track.android",
					"com.joe.track.android.ui.LoginActivity");
			startActivity(i);
			finish();
			return false;
		} else {
			...
			if (mCache.getUser() == null) {
				...
				bundle.putString(TrackApplication.BUNDLEKEY_REDIRECTACTIVITY, getIntent().getClass().getName());
				i.putExtras(bundle);
				startActivity(i);
				finish();
				return false;
	...
	public void updateChatNotification() {
		...
		String username = mCache.getPreferences().getString(TrackApplication.PREFS_USERNAME, "");
		String password = mCache.getPreferences().getString(TrackApplication.PREFS_PASSWORD, "");
		String url = mCache.getServiceUrl();//String.format("%s/user/%d/chat/?count&new",mCache.getProperty("skope_service_url"),userId);
		// Send message
