...
package com.joe.track.android.service;
...
public class WorkerThread extends Thread {
    ...
    /***
     * Calls the Track mLocation service for the given mLocation. 
     *
     * @param bundle
    ...
    private void readUser(final Bundle bundle) {
		...
		// Bundle present, extract mId
    	Long userId = bundle.getLong(TrackApplication.BUNDLEKEY_USERID);
        ...
        Bundle outBundle = new Bundle();
        outBundle.putParcelable(TrackApplication.BUNDLEKEY_USER, user);
    ...
    private void readUserChats(final Bundle bundle) {
		...
		int userId = bundle.getInt(TrackApplication.BUNDLEKEY_USERID);
    ...
    private void readUserChatMessages(final Bundle bundle) {
		...
		int userFromId = bundle.getInt(TrackApplication.BUNDLEKEY_USERID);

		// Check for 'unread' flag
		boolean filterUnreadMessages = false;
		if (bundle.containsKey(TrackApplication.BUNDLEKEY_CHAT_UNREAD)) {
			filterUnreadMessages = bundle.getBoolean(TrackApplication.BUNDLEKEY_CHAT_UNREAD);
		}

		boolean markAsRead = false;
		if (bundle.containsKey(TrackApplication.BUNDLEKEY_CHAT_MARKASREAD)) {
			markAsRead = bundle.getBoolean(TrackApplication.BUNDLEKEY_CHAT_MARKASREAD);
		}
		
		boolean filterFrom = false;
		if (bundle.containsKey(TrackApplication.BUNDLEKEY_CHAT_FROM)) {
			filterFrom = bundle.getBoolean(TrackApplication.BUNDLEKEY_CHAT_FROM);
		}
		
		boolean lastMessage = false;
		if (bundle.containsKey(TrackApplication.BUNDLEKEY_CHAT_LAST)) {
			lastMessage = bundle.getBoolean(TrackApplication.BUNDLEKEY_CHAT_LAST);
		}
        ...
        bundle.putString(TrackApplication.BUNDLEKEY_RESPONSE, response);
